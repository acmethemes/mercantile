...
if ( ! class_exists( 'Mercantile_fdcolumn' ) ) {
	...
	class Mercantile_fdcolumn extends WP_Widget {
		...
		public function form( $instance ) {
			...
			?>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'unique_id' ) ); ?>"><?php _e( 'Section ID', 'mercantile' ); ?>:</label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'page_id' ) ); ?>"><?php _e( 'Select Feature Page', 'mercantile' ); ?>:</label>
				<br />
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'button_text' ) ); ?>"><?php _e( 'Button Text', 'mercantile' ); ?>:</label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'image_align' ) ); ?>"><?php _e( 'Image Align', 'mercantile' ); ?>:</label>
				<select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'image_align' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'image_align' ) ); ?>" >
					...
					foreach ( $mercantile_fd_image_aligns as $key => $value ) {
						?>
						<option value="<?php echo esc_attr( $key ); ?>" <?php selected( $key, $image_align ); ?>><?php echo esc_html( $value ); ?></option>
						<?php
					}
					?>
				</select>
		...
		public function widget( $args, $instance ) {
			...
			if ( ! empty( $page_id ) ) :
				...
				if ( $featured_page_query->have_posts() ) :
					$i = 0;
					while ( $featured_page_query->have_posts() ) :
						$featured_page_query->the_post();
						$title = apply_filters( 'widget_title', esc_html( get_the_title() ), $instance, $this->id_base );
						?>
						<section id="<?php echo esc_attr( $unique_id ); ?>" class="acme-widgets acme-featured-image <?php echo esc_attr( $alternate ); ?>">
							<div class="col-image <?php echo esc_attr( $animation1 ); ?>">
								<?php the_post_thumbnail( 'full', array( 'class' => 'col-img' ) ); ?>
							</div>
							<div class="container <?php echo esc_attr( $animation2 ); ?>">
								<div class="row">
									<div class="col-md-5 <?php echo esc_attr( $offset ); ?>">
										<div class="col-details" style="<?php echo esc_attr( $style ); ?>">
											<?php
											if ( ! empty( $title ) ) {
